 ...
 *
 * Extracted from index.js so it can be unit-tested without initializing the
 * Firebase Admin SDK (see test/cpx_postback.test.js). All external
 * dependencies (Firestore, functions config, FCM, logging) are injected, so
 * the module itself only requires Node's built-in `crypto`.
 */
...
/**
 * Reads the postback secret + dev flag from the Firebase Functions config
 * shape returned by `functions.config()`.
 */
 ...
 * API; `fieldValue` is the FieldValue factory (`admin.firestore.FieldValue`);
 * `messaging` is the FCM client used for the best-effort push.
 *
 ...
 *   getConfig   — () => functions.config() shape
...
async function handleCpxPostback({
  ...
  if (config.allowUnsigned) {
    ...
    if (!config.secret) {
      logger.error(
        'CPX: hash provided but cpx.secret is not configured. ' +
          'Set it with: firebase functions:config:set cpx.secret="..."'
      );
